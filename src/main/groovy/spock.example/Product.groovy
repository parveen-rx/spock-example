...
class Product {
    ...
    static Map<String, List<Product>> getProductsGroupedByPrice(List<Product> products) {
        return products.groupBy {
            if (it.price < 5000) {
                return "Below 5000"
            } else if (it.price >= 5000 && it.price < 10000) {
                return "Between 5001 and 10000"
            } else (it.price >= 10000){
                return "Above 10000"
            }
        }
    ...
    static List<Product> getCurrentProducts(){
        [
                new Product(name: 'p1', price: 100, isPopular: true),
                new Product(name: 'p2', price: 100),
                new Product(name: 'p3', price: 100)
        ] as List<Product>
    }
}
